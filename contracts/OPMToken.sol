...
contract OPMToken is ERC20, ERC20Burnable, Ownable, Pausable, CopyrightToken {
    uint256 public constant MAX_TX = 100_000 * 10**18;
    ...
    constructor(address _router,address _liquidityReceiver) ERC20("One Premium","OPM") {
    ...
    function setExcludedFromLimits(address account,bool excluded) external onlyOwner { _excludedFromLimits[account]=excluded; }
    function setExcludedFromFees(address account,bool excluded) external onlyOwner { _excludedFromFees[account]=excluded; }
    function updateFees(uint256 reflection,uint256 liquidity) external onlyOwner { reflectionFee=reflection; liquidityFee=liquidity; emit FeesUpdated(reflection,liquidity); }
    function addLiquidity(uint256 tokenAmount) external payable onlyOwner {
        _approve(address(this),address(router),tokenAmount);
        router.addLiquidityETH{value:msg.value}(address(this),tokenAmount,0,0,liquidityReceiver,block.timestamp);
        emit LiquidityAdded(tokenAmount,msg.value);
    }
    function burn(uint256 amount) public override { super.burn(amount); }
    function burnFrom(address account,uint256 amount) public override { super.burnFrom(account,amount); }
}
